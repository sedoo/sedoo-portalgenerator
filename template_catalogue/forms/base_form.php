...
function createLoginForm(){
	...
	function createFormBase(){
		...
		$this->addElement ( 'text', 'dats_date_begin', 'Date begin<br>(yyyy-mm-dd)', array (
				'size' => 10 
		) );
		$this->addElement ( 'text', 'dats_date_end', 'Date end<br>(yyyy-mm-dd)', array (
				'size' => 10 
		) );
	...
	function addvalidationRulesBase(){
		...
		$this->addRule ( 'organism_0', 'Contact 1: organism is required', 'couple_not_null', array (
				$this,
				'org_sname_0' 
		) );
		...
		for ($i = 0; $i < $this->dataset->nbPis; $i++){
			...
			if ($i != 0){
				...
				$this->addRule ( 'pi_name_' . $i, 'Contact ' . ($i + 1) . ': organism is required', 'contact_organism_required', array (
						$this,
						$i 
				) );
			}
	...
	function addValidationRulesResolution($prefixMsg = 'Instrument'){
		...
		$this->addRule('sensor_lat_resolution',$prefixMsg.': Latitude coverage exceeds the maximum length allowed (100 characters)','maxlength',100);
		$this->addRule('sensor_lon_resolution',$prefixMsg.': Longitude coverage exceeds the maximum length allowed (100 characters)','maxlength',100);
	}
	...
	function createFormGrid(){
		...
		$this->addElement ( 'textarea', 'grid_process', 'Required grid processing (re-projection, interpolation...) ', array (
				'cols' => 60,
				'rows' => 5 
		) );
	...
	function createFormOrganisme($indice) {
		$org = new organism ();
		$org_select = $org->chargeForm($this,'organism_'.$indice,'Contact '.($indice+1).' organism',$indice);
		$this->addElement($org_select);
		$this->addElement('text','org_sname_'.$indice,'New organism short name: ');
		$this->addElement('text','org_fname_'.$indice,'Full name');
		$this->addElement('text','org_url_'.$indice,'URL');
	}

	function addPi($nb_pi){
		$this->createFormPersonne($nb_pi-1);
	}
	...
	function createFormResolution(){
		$this->addElement('text','sensor_resol_temp','Temporal');
		$this->applyFilter('sensor_resol_temp','trim');
		$this->addElement('text','sensor_vert_resolution','Vertical');
		$this->applyFilter('sensor_vert_resolution','trim');
		//$this->addElement('text','sensor_horiz_resolution','Horizontal');
		//$this->applyFilter('sensor_horiz_resolution','trim');
		$this->addElement('text','sensor_lat_resolution','Latitude');
		$this->applyFilter('sensor_lat_resolution','trim');
		$this->addElement('text','sensor_lon_resolution','Longitude');
		$this->applyFilter('sensor_lon_resolution','trim');
		
	}
	
	function addVariable($nb_variable){
		$this->createFormVariable($nb_variable-1,'');
	}
	
	function createFormVariable($i,$type = ''){

		$key = new gcmd_science_keyword;
		$key_select = $key->chargeForm($this,'gcmd_science_key_'.$type.$i,'Parameter keyword');
		$this->addElement($key_select);

		$this->addElement('hidden','var_id_'.$type.$i);
		$this->addElement('text','new_variable_'.$type.$i,'New parameter name (IF different from the selected parameter keyword)');//,array('onchange' => "resetSelect('variable_".$type.$i."')"));
		$this->applyFilter('new_variable_'.$type.$i,'trim');

		$unit = new unit;
		$unit_select = $unit->chargeForm($this,'unit_'.$type.$i,'Unit',$i,$type);
		$this->addElement($unit_select);
		$this->addElement('text','new_unit_'.$type.$i,'new unit');
		$this->applyFilter('new_unit_'.$type.$i,'trim');
		$this->addElement('text','new_unit_code_'.$type.$i,', unit code: ');
		$this->applyFilter('new_unit_code_'.$type.$i,'trim');
		$this->addElement('textarea','methode_acq_'.$type.$i,'Acquisition methodology and quality',array('cols'=>60, 'rows'=>5));
		$this->applyFilter('methode_acq_'.$type.$i,'trim');
		$this->addElement('text','sensor_precision_'.$type.$i,'Sensor precision');
		$this->applyFilter('sensor_precision_'.$type.$i,'trim');
		
		$this->addElement('text','level_type_'.$type.$i,'Level type');
		$this->applyFilter('level_type_'.$type.$i,'trim');
		...
		$this->addElement('text','var_date_min_'.$type.$i,'Date begin (yyyy-mm-jj)',$options);
		$this->addElement('text','var_date_max_'.$type.$i,'Date end (yyyy-mm-jj)',$options);
		...
		if (isset($this->dataset->projects)){
			foreach ($this->dataset->projects as $proj){
				if (isset($proj)){
					...
					$this->setDefaults(array('project_'.$indice++ => array($p1,$p2,$p3)));
				}
			}
		}
	...
	function initFormResolution(){
		if(isset($this->dataset->dats_sensors[0]->sensor_resol_temp) && !empty($this->dataset->dats_sensors[0]->sensor_resol_temp))
	...
	function saveFormResolution(){
		...
		//$this->dataset->dats_sensors[0]->sensor_horiz_resolution = $this->exportValue('sensor_horiz_resolution');
